...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const monthlyChartData = JSON.parse(columnChart.getAttribute('data-monthly-chart'));

    const categories = [];
    const seriesReceipts = [];
    const seriesCosts = [];
    const seriesProfit = [];

    for (let i = 0; i < monthlyChartData.length; i++) {
        const data = monthlyChartData[i];
        categories.push(data.month);
        seriesReceipts.push(data.totalReceipt);
        seriesCosts.push(data.totalCost);
        seriesProfit.push(data.netProfit);
    }
...
////////////// ANUAL
document.addEventListener('DOMContentLoaded', function() {
    const yearlyColumnChart = document.getElementById('yearly_column_chart');
    const yearlyChartData = JSON.parse(yearlyColumnChart.getAttribute('data-yearly-chart'));
    ...
    const categories = yearlyChartData.map(data => months[data.month - 1]);
    const seriesReceipts = yearlyChartData.map(data => data.totalReceipt);
    const seriesCosts = yearlyChartData.map(data => data.totalCost);
    const seriesProfit = yearlyChartData.map(data => data.netProfit);
    ...
    const chart = new ApexCharts(
        document.querySelector("#yearly_column_chart"),
        options
    );
    chart.render();
});
